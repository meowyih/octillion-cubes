...
std::error_code octillion::SslServer::start( std::string port, std::string key, std::string cert )
{
    ...
    core_thread_ = new std::thread( &SslServer::core_task, this );
...
void octillion::SslServer::core_task()
{
    ...
    while( core_thread_flag_ )
    {
        ...
        for (auto it = out_data_.begin(); it != out_data_.end(); ) 
        {
            ...
            std::map<int, Socket>::iterator itsocket = socket_list_.find( (*it).fd );
            
            if( itsocket == socket_list_.end() )
            {
                // something really bad happens
                LOG_E(tag_) << "Error, client fd " << (*it).fd 
                   << " in out_data_ does not exist in socket_list_";
            ...
            ret = SSL_write( itsocket->second.ssl, (*it).data, (*it).datalen );
        ...
        {   
            ...
            if ( events[i].events & EPOLLOUT )
            {
                ...
                iter = socket_list_.find( events[i].data.fd );
                
                if( iter == socket_list_.end() )
                {
                    // something really bad happens
                    LOG_E(tag_) << "Error, client fd " << events[i].data.fd 
                       << " does not exist in socket_list_";
                    requestclosefd( events[i].data.fd );
                }
                else
                {
            ...
            if ( server_fd_ == events[i].data.fd )
            {
                ...
                while( true )
                {
                        ...
                        SSL_get_error(ssl, ret) == SSL_ERROR_WANT_WRITE ))
                    {
                        ...
                        socket.ssl = ssl;
                        socket_list_.insert( std::pair<int, Socket>(infd, socket) );
            ...
            else
            {
                ...
                std::map<int,Socket>::iterator it = socket_list_.find( events[i].data.fd );
    ...
    for (const auto& data : socket_list_ ) 
    {
...
std::error_code octillion::SslServer::senddata( int fd, const void *buf, size_t len )
{
...
void octillion::SslServer::closesocket( int fd )
{
    ...
    // clean up ssl
    std::map<int, Socket>::iterator iter = socket_list_.find( fd );
    if ( iter != socket_list_.end() )
    {
    ...
    {
        LOG_E(tag_) << "closesocket fd:" << fd << " does not exist in socket_list_";
    }
